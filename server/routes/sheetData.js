...
router.get('/', async (req, res) => {
  try {
    const usersRes = await db.query('SELECT iniciales FROM users ORDER BY orden ASC, iniciales ASC');
    ...
    if (start && end) {
      const fechasRes = await db.query(
        `
          SELECT DISTINCT fecha FROM filas_fecha WHERE fecha >= $1 AND fecha <= $2
          UNION
          SELECT DISTINCT fecha FROM inscripciones WHERE fecha >= $1 AND fecha <= $2
          ORDER BY fecha
        `,
        [start, end]
      );
      fechas = fechasRes.rows.map(r => r.fecha);
    } else {
      const fechasRes = await db.query(
        `
    ...
    if (fechas.length) {
      const inscRes = await db.query(
        `
          SELECT fecha, comida, iniciales, opcion
          FROM inscripciones
          WHERE fecha >= $1 AND fecha <= $2
        `,
        [fechas[0], fechas[fechas.length - 1]]
      );
      inscRes.rows.forEach(r => {
